	...
	public function init() {
		...
		$this->advanced_fields = $this->base_advanced_fields(
			array(
				...
				'tab' => array( 'label' => esc_html__( 'Tabs', 'honest-divi-modules' ), 'css' => array( 'main' => "{$this->main_css_element} .honest-market__tab" ), 'toggle_slug' => 'tab', 'hide_text_color' => true, 'font_size' => array( 'default' => '24px' ), 'font' => array( 'default' => '|700|||||||' ), 'line_height' => array( 'default' => '1em' ), 'letter_spacing' => array( 'default' => '0px' ) ),
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		foreach ( $markets as $i => $market ) {
			...
			foreach ( honest_team_get_members( $market['members'] ) as $member ) {
				...
				// Cards ship hidden so market-map.js can time their entrance against
				// the map, but only on the front end: in the builder that script
				// never reaches these nodes and the grid would render empty.
				$cards .= honest_team_render_member_card(
					$member,
					null,
					honest_team_is_builder_render() ? '' : 'honest-market-hold'
				);
			}
